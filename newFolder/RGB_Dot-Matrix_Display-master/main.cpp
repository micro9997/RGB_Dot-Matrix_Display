...
void accessDisplay(uint8_t comm, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    data = 0b11110000000011110000000000000000;

    for(int i = 0; i < 1; i++) {
        firstSR.shiftOut(LSBFIRST, (data >> (32 * i)));
    }

    // firstSR.shiftOut(LSBFIRST, data);

    // firstSR.shiftOut(LSBFIRST, (data >> 0));
    // firstSR.shiftOut(LSBFIRST, (data >> 8));
    // firstSR.shiftOut(LSBFIRST, (data >> 16));
    // firstSR.shiftOut(LSBFIRST, (data >> 24));
